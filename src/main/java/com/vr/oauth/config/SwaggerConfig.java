...
public class SwaggerConfig {
	...
	public Docket projectApi() {
		return new Docket(DocumentationType.SWAGGER_2).ignoredParameterTypes(ignoreParamClasses()).groupName("rest-api")
				.apiInfo(apiInfo()).select()
				.apis(Predicates.not(RequestHandlerSelectors.basePackage("org.springframework"))).build();
	}

	private ApiInfo apiInfo() {
		return new ApiInfoBuilder().title("Learn Spring Oauth Security").description("Learn Spring Oauth security")
				.termsOfServiceUrl("https://github.com/sachinnagalgawe").contact(new Contact("Sachin Nagalgawe",
						"https://github.com/sachinnagalgawe", "sachinnagalgawe@gmail.com"))
				.build();
	}
