...
public final class AttributeProvider {
	...
	/**
	 * Fetch all the fields marked with @DefaultField annotations using reflection
	 * and set into the collection passed to method.
	 */
		...
		if (defaultFields != null) {
			for (Field field : defaultFields) {
				...
				if (isCustomObject(field)) {
					...
					fieldName = (!StringUtils.isEmpty(parentField)) ? parentField + "." + fieldName : fieldName;
