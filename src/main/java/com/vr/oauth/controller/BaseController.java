...
public class BaseController {
	...
	private <T> T adjustFields(T data, Class<T> clazz, String[] excludeFields, String[] includeFields) {
		return json.use(JsonView.with(data).onClass(clazz, Match.match().exclude(excludeFields).include(includeFields)))
				.returnValue();
	}

	private <T> List<T> adjustFields(List<T> data, Class<T> clazz, String[] excludeFields, String[] includeFields) {
		return json.use(JsonView.with(data).onClass(clazz, Match.match().exclude(excludeFields).include(includeFields)))
				.returnValue();
	}
	...
	protected <T> List<T> limitDataFields(List<T> data, Class<T> clazz) {
		return adjustFields(data, clazz, FieldConstants.EXCLUDEALL_FIELDS, AttributeProvider.provideFields(clazz, ""));
	}
